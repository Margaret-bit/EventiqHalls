...
import { FiPlus, FiPackage, FiX, FiMapPin, FiUsers, FiCheck, FiUpload } from "react-icons/fi"
...
const PlanCard = styled.div`
  background: #F3F0FF;
  border: ${(props) => (props.popular ? "2px solid #7c3aed" : "2px solid #e0e0e0")};
  border-radius: 12px;
  padding: 2rem 1.5rem;
  text-align: center;
  position: relative;
...
const PlanDuration = styled.h3`
  ...
  font-weight: 600;
  color: #333;
  margin: ${(props) => (props.popular ? "1.5rem 0 0.75rem 0" : "0 0 0.75rem 0")};
`
...
const Tab = styled.button`
  ...
  border-radius: 8px;
  transition: all 0.2s;
  border: ${(props) => (props.active ? "1px solid #7c3aed" : "1px solid transparent")};
...
const VenueCardSkeletonLoader = () => (
  <VenueCardSkeleton>
    <SkeletonImage />
    <SkeletonContent>
      <SkeletonBar style={{ width: "80%", height: "24px", marginBottom: "12px" }} />
      <SkeletonBar style={{ width: "100%", height: "16px", marginBottom: "8px" }} />
      <SkeletonBar style={{ width: "90%", height: "16px", marginBottom: "8px" }} />
...
const Venues = () => {
  ...
  const fetchVenues = async () => {
    try {
      ...
      const response = await axios.get("https://eventiq-final-project.onrender.com/api/v1/venues", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      })

      console.log("[v0] Venues API Response:", response)
      console.log("[v0] Response Status:", response.status)
      console.log("[v0] Response Data:", response.data)
      console.log("[v0] Venues Count:", response.data?.data?.length || 0)
      console.log("[v0] Current User ID:", userId)

      if (response.data && response.data.data) {
        const userVenues = userId
          ? response.data.data.filter((venue) => venue.venueOwnerId === userId)
          : response.data.data

        console.log("[v0] Filtered User Venues:", userVenues)
      ...
      console.error("[v0] Venues fetch error:", err)
  ...
  const handleOpenModal = (venue = null) => {
    ...
    if (venue) {
      ...
      setUploadedImages(venue.images?.map((img) => ({ url: img.url, publicId: img.publicId })) || [])
      setDocumentFiles({
  ...
  const handleImageUpload = (e) => {
    ...
    if (files.length > remainingSlots) {
      toast.warning(`Only ${remainingSlots} image(s) can be added. Maximum is 5 images.`)
    }
  }
  ...
  const handleDocumentUpload = (e, documentType) => {
    ...
    if (file) {
      ...
      toast.success(`${documentType === "cacCertificate" ? "CAC" : "Certificate of Occupancy"} uploaded`)
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    if (!documentFiles.cacCertificate || !documentFiles.certificateOfOccupancy) {
      toast.error("Please upload both CAC Certificate and Certificate of Occupancy")
      setActiveTab("documents")
      return
    }
    ...
    try {
      ...
      if (err.code === "ERR_NETWORK") {
        ...
        toast.error(
          typeof err.response.data === "string" ? err.response.data : err.response.data.message || "Validation error",
        )
  ...
  const handleDelete = async (venueId) => {
    if (window.confirm("Are you sure you want to delete this venue?")) {
      try {
        ...
        await axios.delete(`https://eventiq-final-project.onrender.com/api/v1/venues/${venueId}`, {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
  ...
  const handleSubscribe = async (featureId) => {
    try {
      ...
      const response = await axios.get(`https://eventiq-final-project.onrender.com/api/v1/subscribe/${featureId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    const fetchFeatures = async () => {
      try {
        ...
        const response = await axios.get("https://eventiq-final-project.onrender.com/api/v1/features", {
          headers: {
            Authorization: `Bearer ${token}`,
          },
        })
    ...
    <VenuesContainer>
      ...
      {loading ? (
        ...
        <EmptyState>
          ...
          <EmptyDescription>Upload your venue details to get noticed</EmptyDescription>
        </EmptyState>
      ) : (
        <>
          <VenuesGrid>
                ...
                <VenueImageWrapper>
                  {venue.images && venue.images.length > 0 ? (
                    <VenueImage
                      ...
                      onLoad={() => {
                        console.log("[v0] Image loaded successfully:", venue.images[0]?.url)
                      }}
                    />
                  ) : (
                ...
                <VenueContent>
                  ...
                  <VenueStats>
                    <StatItem>
                      ...
                      <StatValue>₦{(venue.price || 0).toLocaleString()}</StatValue>
                    </StatItem>
                    <StatItem>
                  ...
                  <ViewDetailsButton onClick={() => handleOpenModal(venue)}>Edit Details</ViewDetailsButton>
                </VenueContent>
                <VenueActions>
          ...
          <PlansContainer>
            ...
            <PlansSubtitle>Boost your venue's visibility and get more bookings</PlansSubtitle>
            <PlansGrid>
      ...
      {isModalOpen && (
        <Modal onClick={handleCloseModal}>
          <ModalContent onClick={(e) => e.stopPropagation()}>
            <ModalHeader>
              <ModalTitle>{editingVenue ? "Edit Venue" : "Add New Event Venue"}</ModalTitle>
            ...
            {activeTab === "basic" && (
              <FormContainer>
                ...
                <FormGroup>
                  ...
                  <HelpText>Select all amenities available at your venue</HelpText>
                  <CheckboxGrid>
                    <CheckboxLabel>
                      <input
                        type="checkbox"
                    ...
                    <CheckboxLabel>
                      <input
                        ...
                        checked={formData.amenities.includes("Professional Lighting")}
                        onChange={handleInputChange}
                      />
                    ...
                    <CheckboxLabel>
                      <input
                        ...
                        checked={formData.amenities.includes("Projector/Screen")}
                        onChange={handleInputChange}
                      />
                      Projector/Screen
                    </CheckboxLabel>
                    <CheckboxLabel>
                      <input
                        ...
                        checked={formData.amenities.includes("Catering Services")}
                        onChange={handleInputChange}
                      />
                      Catering Services
                    </CheckboxLabel>
                    ...
                    <CheckboxLabel>
                      <input
                        ...
                        checked={formData.amenities.includes("Backup Generator")}
                        onChange={handleInputChange}
                      />
                      Backup Generator
                    </CheckboxLabel>
                    ...
                    <CheckboxLabel>
                      <input
                        ...
                        checked={formData.amenities.includes("Kitchen Facilities")}
                        onChange={handleInputChange}
                      />
                      Kitchen Facilities
                    </CheckboxLabel>
                ...
                <BottomActions>
                  <NextButton onClick={() => setActiveTab("documents")}>Next: Documents & Images</NextButton>
                </BottomActions>
              </FormContainer>
            )}
            ...
            {activeTab === "documents" && (
              <FormContainer>
                <FormGroup>
                  ...
                  <HelpText>Upload at least 1 and maximum 5 images/videos of your venue</HelpText>
                  <ImageUploadSection>
                    <input
                      type="file"
                      multiple
                    ...
                    <label
                      htmlFor="image-input"
                      style={{ cursor: uploadedImages.length >= 5 ? "not-allowed" : "pointer" }}
                    >
                      <ImageUploadIcon>⬆️</ImageUploadIcon>
                      <ImageUploadText>
                        <strong>Click to upload</strong> or drag and drop
                      </ImageUploadText>
                      <ImageUploadHelper>PNG, JPG, MP4 up to 10MB each (Max 5 images)</ImageUploadHelper>
                    </label>
                  </ImageUploadSection>
                  {uploadedImages.length > 0 && (
                    <>
                      <ImagePreviewContainer>
                        {uploadedImages.map((img, index) => (
                          <ImagePreviewItem key={index}>
                            <img src={img.url || "/placeholder.svg"} alt={`Preview ${index}`} />
                      ...
                      <HelpText>{uploadedImages.length}/5 images uploaded</HelpText>
                    </>
                  )}
                </FormGroup>
                ...
                <KYCSection>
                  ...
                  <HelpText>Upload required business documents for verification</HelpText>

                  <DocumentUploadGroup>
                    <DocumentLabel>
                      CAC Certificate (Corporate Affairs Commission) <span>*</span>
                    </DocumentLabel>
                    <ImageUploadSection>
                      <input
                        type="file"
                        accept=".pdf,.doc,.docx,.jpg,.png"
                        onChange={(e) => handleDocumentUpload(e, "cacCertificate")}
                        style={{ display: "none" }}
                        id="cac-input"
                      />
                      <label htmlFor="cac-input" style={{ cursor: "pointer" }}>
                        ...
                        <ImageUploadHelper>PDF or Image (Max 5MB)</ImageUploadHelper>
                      </label>
                    </ImageUploadSection>
                  </DocumentUploadGroup>
                  ...
                  <DocumentUploadGroup>
                    ...
                    <ImageUploadSection>
                      <input
                        type="file"
                        accept=".pdf,.doc,.docx,.jpg,.png"
                        onChange={(e) => handleDocumentUpload(e, "certificateOfOccupancy")}
                        style={{ display: "none" }}
                        id="coo-input"
                      />
                ...
                <BottomActions>
                  <PreviousButton onClick={() => setActiveTab("basic")}>Previous</PreviousButton>
